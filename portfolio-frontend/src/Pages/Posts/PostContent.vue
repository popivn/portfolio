<template>
    <div class="bg-gradient-secondary rounded-lg shadow-lg w-full relative overflow-hidden">
      ...
      <button 
        @click="$emit('close')"
        class="absolute top-4 left-4 z-10 w-8 h-8 flex items-center justify-center rounded-full bg-white/80 text-gray-700 hover:bg-white transition-colors"
      >
      ...
      <div class="w-full h-48 bg-gray-200 relative">
        <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/70 flex flex-col justify-end p-6">
          ...
          <div class="flex items-center justify-between mt-2">
            <p class="text-white/90">By {{ post.author }}</p>
            <p class="text-white/90">{{ formatDate(post.time) }}</p>
          </div>
        </div>
      </div>
